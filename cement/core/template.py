...
class TemplateInterface(Interface):
    ...
    @abstractmethod
    def copy(self, src: str, dest: str, data: Any) -> bool:
        """
    ...
    @abstractmethod
    def load(self, path: str) -> Tuple[Optional[Union[str, bytes]],
                                       str, Optional[str]]:
        """
...
class TemplateHandler(TemplateInterface, Handler):  # type: ignore
    """
    ...
    def copy(self, src: str, dest: str, data: Any, force: bool = False,
        ...
        coded_src_pattern = escaped_src.encode('unicode-escape')
        escaped_src_pattern = coded_src_pattern.decode('utf-8')
        ...
        for cur_dir, sub_dirs, files in os.walk(src):
            ...
            for sub_dir in sub_dirs:
                coded_sub_dir = sub_dir.encode('unicode-escape')
                escaped_sub_dir = coded_sub_dir.decode('utf-8')
    ...
    def _load_template_from_file(
            self,
            template_path: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _load_template_from_module(
            self,
            template_path: str) -> Tuple[Optional[bytes], Optional[str]]:
    ...
    def load(self, template_path: str) -> Tuple[Union[str, bytes], str,
                                                Optional[str]]:
        """
        Loads a template file first from ``self.app._meta.template_dirs`` and
        secondly from ``self.app._meta.template_module``.  The
